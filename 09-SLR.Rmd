...
```
t value = 9.464,Il coefficiente della velocità è 9.464 “errori standard” lontano da 0
...
```
test F significativo, quindi possiamo concludere che il nostro modello F è soddisfacente,
quindi la varianza spiegata è significativamente più grande di quella non spiegata.
3.93 è circa la pendenza della retta, di cui la sua incertezza è misurata dallo standard error
...
andiamo a vedere l'intervallo di confidenza sia per l'intercetta sia per beta1, ossia il coefficiente angolare della velocità. L'intervallo di confidenza ci rappresenta il fatto che nel 95% dei casi quell'intervallo conterrebbe il valore vero di beta1.
...
```
questo output grafico riporta gli intervalli di confidenza al 95% della nostra regressione. Vediamo nel grafuco sia l'intervallo di confidenza per la regressione (linea tratteggiata verde) L'altro intervallo di confidenza, quello della prediction, è quello determinato dalle linee tratteggiate rosse. Se noi dovessimo prevedere un nuovo punto, questo al 95% dei casi si troverebbe nelle linee rosse, se andassimo a misurarlo, mentre ci aspettiamo che la nostra retta si trovi bene o male all'interno delle linee tratteggiate verdi.
...
Let's check if the data support this statement. (Try to formulate the null and alternative hypothesis first!). NOn vogliamo solo verificare se tale valore di b1 è diverso da 0 o meno, ma anche se è o meno uguale a 1. Se fosse casualmente uguale a 1 vorrebbe dire che di anno in anno nei pazienti il battito cardiaco aumenta di una unità.
...
confint(lm.result)
```
13 gradi di libertà perchè ci sono due parametri (intercetta e coefficiente angolare, ci sono due parametri che sottraggono libertà al modello e che stiamo andando a stimare)
x è il coefficiente angolare, ci viene -0.79 e vogliamo testare se esso è o meno significativamente diverso da 1. In realtà il battito diminuisce secondo il nostro grafico.
I dati disponibili però ci dimostrano che c'è una relazione negativa e il nostro coefficiente è negativo. 
modifichiamo ora la ipotesi nulla, in modo che quella alternativa sia che b1 è diversa significativamente da -1 e testiamo questa ipotesi.
...
b1 = -0.79773# coefficiente angolare
SE = 0.06996
...
```
usiamo pt anzichè t.test perchè vogliamo testare se t è significativamente diverso da -1. t va calcolato a mano perchè lo vogliamo calcolare noi ed è diverso da quello che R calcolerebbe di default.
lower.tail=FALSE e moltiplicare per due se vogliamo fare un test a due code. Scopro che il valore che ho trovato è significativamente diverso da -1.
...
```
stessa cosa se volessimo testare il valore di beta0, intercetta.
