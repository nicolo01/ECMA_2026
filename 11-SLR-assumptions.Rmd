...
```
vale la pena fare un test di shapiro perchè nel grafico precedente vedevamo che ad un certo punto i dati iniziano a disperdersi rispetto alla retta, e facendolo effettivamente si nota che il p-value è significativo, nonostante W , il valore della statistica di Shapiro è quasi uguale a 1, come lo vorremmo noi. PErò possiamo scendere a compromessi e decidere cmq che non è troppo importante questo valore (inoltre questo è l'assunto meno importante tra tutti)
...
```
valore di breush-pagan non significativo quindi possiamo stare tranquillo. Vediamo adesso cosa dobbiamo fare se venisse storto qualcosa.
...
trees %>%
  mutate(Volume.bc=Volume^0.31) ->trees.bc #aggiungiamo una colonna (Volume.bc e calcola come colonna volume originaria elevata a 0.31) al dataset originario
